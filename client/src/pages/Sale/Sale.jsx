...
const Sale = () => {
  ...
  const filteredProducts = products.filter(product => {
    ...
    // Price filter
    if (priceParam) {
      const price = product.discountPrice > 0 ? (product.price - product.discountPrice) : product.price;
    ...
    <div className="min-h-screen bg-[#F8FAFC] pt-8 pb-24 px-4 md:px-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-12 text-center md:text-left">
          ...
          >
            Flash <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#D99A20] to-[#F5B942]">Sale</span>
          </motion.h1>
          <motion.p 
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            Grab your favorite footwear at unbeatable prices. Limited time offers on top styles.
          </motion.p>
        </div>
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-32">
            ...
            <p className="text-[#111827]/60 font-bold uppercase tracking-widest text-xs">Loading sales...</p>
          </div>
        ) : error ? (
          ...
          <div className="text-center py-32 rounded-[3rem] bg-white border border-dashed border-[#CBD5E1] shadow-sm my-10">
            <h3 className="text-2xl font-black uppercase italic text-[#071A2F]/60 mb-2">No items on sale</h3>
